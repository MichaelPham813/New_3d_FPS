...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float maxSpeed = 50f;// cao nhất cho wall running và sliding
    ...
    Vector3 counterScale = new Vector3(1,2,1);
    ...
    private void StartCrouch()
    {
        ...
        playerCamera.localScale = counterScale;
    ...
    private void StopCrouch()
    {
        ...
        playerCamera.localScale = playerScale;
        transform.position = new Vector3(transform.position.x, transform.position.y + 0.5f, transform.position.z);
        sliding = false;
    }
    ...
    public void Movement()
    {
        ...
        if(Grappler.GetComponent<Grappling>().isGrappling){
                ...
                if(Input.GetKey(KeyCode.LeftShift)){
                    ...
                    if(Vector3.Distance(transform.position,Grappler.GetComponent<Grappling>().GetGrapplePoint())>0.1f){
                        rb.AddForce(Grappler.GetComponent<Grappling>()
                            .GetGrapplePoint()-transform.position * multiplier * multiplierV, ForceMode.Impulse);
                            //transform.RotateAround(Grappler.GetComponent<Grappling>().GetGrapplePoint(),dir,multiplier*100*Time.deltaTime);
                    }
    ...
    private void StartWallrun()
    {
        ...
        if (rb.velocity.magnitude <= 25f + maxWallSpeed)
        {
            ...
            //làm nhân vật chắc chắn dính vào tường
            if (isWallRight)
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            Hit();
            gameObject.GetComponent<TakeDamage>().TakeDamages(MeleeEnemy.FindObjectOfType<MeleeEnemy>().damagedeal);
            gameObject.GetComponent<TakeDamage>().TakeDamages(FastEnemy.FindObjectOfType<FastEnemy>().damagedeal);
        }
    }
